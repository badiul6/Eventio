            ...
            <form id="profileForm" action="{{route('trainee.update')}}" method="post">
                ...
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="fname" value="{{$train->first_name}}" id="fname" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 required="">
                    </div>
                    <div>
                        <label for="lname" class="block mb-2 text-sm font-medium text-gray-900 ">Last Name</label>
                        <input id="lname"name="lname" value="{{$train->last_name}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 ">
          
                    </div>
                    
                    <div>
                        <label for="contact" class="block mb-2 text-sm font-medium text-gray-900 ">Contact No</label>
                        <input type="contact" name="contact" value="{{$train->phone_no}}" id="contact" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 required="">
                    </div>
                    <div>
                        <label for="experience" class="block mb-2 text-sm font-medium text-gray-900 ">Years of Experience</label>
                        <input id="experience" name="experience" value="{{$train->experience}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 ">
                    </div>
                    ...
                    <div class=" sm:col-span-2">
                        ...
                        <textarea id="bio" name="bio" rows="2"  class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 here">{{$train->bio}}</textarea>
                    </div>
                </div>
                @if ($errors->any())
